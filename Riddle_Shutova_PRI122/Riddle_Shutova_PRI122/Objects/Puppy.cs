...
namespace Riddle_Shutova_PRI122.Objects
{
    public class Puppy
    {
        ...
        public void Draw()
        {
            ...
            // Правее дочки на 5.8f
            GL.PushMatrix();
            GL.Translate(5.8f, -1.0f + JumpOffset, 0);
            GL.Rotate(RotationY, 0, 1, 0);

            // 1. Тело - маленькая капсула белого цвета 
            GL.Color3(1.0f, 1.0f, 1.0f);
            GL.PushMatrix();
            GL.Translate(0, -0.1f, 0); 
            GL.Rotate(90, 0, 1, 0);
            DrawCapsule(0.5f / 3, 2.0f / 3, 20); 
            GL.PopMatrix();

            // 2. Лапы - четыре маленькие капсулы белого цвета
            // Передние лапы
            GL.PushMatrix();
            GL.Translate(0.7f / 3, -0.8f / 3, 0.3f / 3);
            GL.Rotate(90, 1, 0, 0);
            DrawCapsule(0.15f / 3, 0.6f / 3, 20);
            GL.PopMatrix();
            ...
            // 3. Голова - маленький шар белого цвета 
            GL.PushMatrix();
            GL.Translate(1.2f / 3, 0.2f / 3, 0);
            DrawSphere(0.4f / 3, 20, 20);
            GL.PopMatrix();

            // 4. Уши - приплюснутые эллипсы коричневого цвета
            GL.Color3(0.6f, 0.4f, 0.2f);

            // Левое ухо 
            GL.PushMatrix();
            GL.Translate(0.8f / 3, 0.6f / 3, 0.3f / 3); 
            GL.Rotate(50, 0, 0, 1); 
            GL.Rotate(-10, 1, 0, 0); 
            DrawEllipsoid(0.3f / 3, 0.5f / 3, 0.02f, 20, 20); 
            GL.PopMatrix();
            ...
            // 5. Нос - маленький белый эллипсоид с отсечённой верхней половиной и закрытый сверху диском
            GL.Color3(1.0f, 1.0f, 1.0f);
            GL.PushMatrix();
            GL.Translate(1.5f / 3, 0.2f / 3, 0);
            GL.Rotate(90, 1, 0, 0);

            DrawHalfEllipsoid(0.4f / 3, 0.3f / 3, 0.3f / 3, 20, 20);

            GL.PushMatrix();
            GL.Translate(0, 0, -0.046f / 3);
            DrawEllipticalDisk(0.4f / 3, 0.3f / 3, 20);
            GL.PopMatrix();

            GL.PopMatrix();

            // 6. Хвост - приплюснутая коричневая усечённая пирамида
            GL.Color3(0.6f, 0.4f, 0.2f);
            GL.PushMatrix();
            GL.Translate(-1.6f / 3, 0.8f / 3, 0); 
            GL.Rotate(220, 0, 0, 1); 
            GL.Rotate(-90, 0, 1, 0);
            DrawTrapezoid(0.5f / 3, 0.1f / 3, 1.1f / 3, 0.02f, 20); 
            GL.PopMatrix();
